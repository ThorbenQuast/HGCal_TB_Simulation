...
private:
  ...
  std::map<int, TH1D*>                     h_cell_energy;
  ...
  double                                   delta_X, delta_Y, zMaxPrimaryParticle;
  bool                                     valid_reference_mimick;
};
...
void HGCalTBAnalyzer::beginJob() {
  ...
  int nLayers = 7+10+12; //September 2017: 7 in EE, 10 in FH, 12 for BH
  for( int ilayer=1; ilayer<=nLayers; ilayer++ ){
    os.str("");
    os << "Layer" << (ilayer);
    TFileDirectory layer_dir = fs_->mkdir( os.str().c_str() );
    for (int icellno=0; icellno<=133; icellno++) {
      int key = ilayer*1000 + icellno;
      os.str("");
      os << "Energy_cell" << icellno;
      h_cell_energy[key] = layer_dir.make<TH1D>(os.str().c_str(), os.str().c_str(), 1500, 0., 1500.);   //unit is GeV*1,000,000 = keV
    }
  ...
  if (doSimHits_ && doTree_) {
    ...
    tree_->Branch("delta_X",         &delta_X,           "delta_X/D");
    tree_->Branch("delta_Y",         &delta_Y,           "delta_Y/D");
    tree_->Branch("zMaxPrimaryParticle",         &zMaxPrimaryParticle,           "zMaxPrimaryParticle/D");
    if (doTreeCell_) {
...
void HGCalTBAnalyzer::analyzeSimHits (int type, std::vector<PCaloHit>& hits,
  ...
  for (auto itr : map_hitn) {
    ...
    if (!fillCellEnergyHisto) continue;
...
void HGCalTBAnalyzer::analyzeSimTracks(edm::Handle<edm::SimTrackContainer> const& SimTk, 
  ...
  double _S = 0., _S_x = 0., _S_y = 0., _S_z = 0., _S_xz = 0., _S_yz = 0., _S_zz = 0.;
  int N_points_for_trajectory = 0;

  zMaxPrimaryParticle = -9999999.;
  

  for (simVtxItr = SimVtx->begin(); simVtxItr!= SimVtx->end(); simVtxItr++) {

    zMaxPrimaryParticle = std::max(zMaxPrimaryParticle, simVtxItr->position().Z());
    if (simVtxItr->position().Z() < 800. || simVtxItr->position().Z() > 1000.) continue;    //interested in the overall slope prior to the first layer before z=1110 [cm]

    _S += 1.0;
    _S_x += simVtxItr->position().X();
    _S_y += simVtxItr->position().Y();
    _S_z += simVtxItr->position().Z();
    _S_xz += simVtxItr->position().X() * simVtxItr->position().Z();
    _S_yz += simVtxItr->position().Y() * simVtxItr->position().Z();
    _S_zz += simVtxItr->position().Z() * simVtxItr->position().Z();

    N_points_for_trajectory++;
  }

  if (N_points_for_trajectory>=3) {

    //angular study
    double _Delta = _S * _S_zz - _S_z * _S_z;
    //double m_x = (_S*_S_xz-_S_z*_S_x)/_Delta;
    //double m_y = (_S*_S_yz-_S_z*_S_y)/_Delta;
    double b_x = (_S_zz*_S_x-_S_z*_S_xz)/_Delta;
    double b_y = (_S_zz*_S_y-_S_z*_S_yz)/_Delta;

    delta_X = b_x - xBeam_;
    delta_Y = b_y - yBeam_;

    valid_reference_mimick = true;

  } else {
    delta_X = 0.;
    delta_Y = 0.;

    valid_reference_mimick = false;
  }
